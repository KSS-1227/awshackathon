...
async def extract_entities(
    ...
    # NetworkXStorage (used here for image extraction) does not expose the batch
    # API, so always use the scalar path. The batch path is available when
    # CockroachGraphStorage is wired in (e.g. future image pipeline changes).
    if _supports_batch(knwoledge_graph_inst):
        all_entities = await _merge_nodes_batch(dict(maybe_nodes), knwoledge_graph_inst)
        await _merge_edges_batch(dict(merged_edges), knwoledge_graph_inst)
    else:
