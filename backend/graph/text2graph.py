...
async def extract_entities(
    ...
    # Use the batched path for CockroachDB (1 SELECT + 1 executemany per call).
    # Fall back to the per-entity path for NetworkXStorage or any backend that
    # doesn't expose the batch API (e.g. tests using a mock store).
    if _supports_batch(knwoledge_graph_inst):
        entities_data = await _merge_nodes_batch(dict(all_nodes), knwoledge_graph_inst)
        await _merge_edges_batch(dict(all_edges), knwoledge_graph_inst)
    else:
...
class TextEntityExtractor:
    ...
    def __post_init__(self):
        # workspace_id is optional now that we support NetworkXStorage
