...
class WorkspaceDocumentService:
    ...
    async def process_documents(
        ...
        All files are preprocessed concurrently (parallel I/O + LLM chunking)
        then merged into the graph sequentially.  A single embeddings pass and
        output save runs once after all files are processed.
        ...
        # index_many() handles deduplication, concurrent preprocessing, and
        # sequential graph merging.  It returns {filename: status_string}.
        batch_results = await builder.index_many(file_paths)
        ...
        for file_path in file_paths:
            ...
            status = batch_results.get(fname, "unknown")
            if status in ("processed", "skipped"):
                processed.append(fname)
                proc_results.append({"file": fname, "status": status})
            else:
                # status is "failed: <reason>" or "unknown"
                reason = status.removeprefix("failed: ") if status.startswith("failed") else status
                failed.append({"file": fname, "error": reason})
                proc_results.append({"file": fname, "status": "failed", "error": reason})
