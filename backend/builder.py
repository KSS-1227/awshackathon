...
--------------------
index_many(file_paths) is the preferred entry point when processing several
files at once.  It runs _step_preprocessing() for all files concurrently
(expensive I/O + LLM chunking), then processes each file's graph-merge stage
sequentially so shared JSON stores and CockroachDB are never written from
parallel coroutines.  _step_embeddings / _step_save_output / _step_generate_report
are called once after all files have been merged.
"""
import asyncio
import hashlib
import logging
...
class MMKGBuilder:
    ...
    async def index_many(self, file_paths: list[str]) -> dict[str, str]:
        ...
        files because it is purely I/O-bound and each file's chunks land in
        independent in-memory structures before being merged into the shared
        KV store.

        Graph merging (entity extraction, image extraction, fusion, Cockroach
        upserts) runs sequentially per file because it writes to shared JSON
        stores and CockroachDB rows.

        _step_embeddings / _step_save_output / _step_generate_report are called
        once after all files have been merged, avoiding redundant full-graph
        operations.

        Parameters
        ----------
        file_paths:
            Absolute or workspace-relative paths to the files to ingest.
        ...
        # are idempotent as long as they don't interleave mid-write.
        # asyncio.gather runs coroutines on the same event-loop thread so
        # there is no true parallelism and no file-write races.
        async def _preprocess_safe(fp: str) -> tuple[str, str | None]:
            """Returns (file_path, error_message_or_None)."""
            try:
        ...
        preprocess_outcomes = await asyncio.gather(
            *[_preprocess_safe(fp) for fp in to_process]
        )

        # Separate successes from failures after preprocessing
        preprocessing_failed: set[str] = set()
        for fp, err in preprocess_outcomes:
            if err is not None:
        ...
        for fp in to_process:
            ...
            fname = Path(fp).name
            try:
                had_images = await self._step_graph_merge(fp)
                if had_images:
                    any_images = True
    ...
    async def _step_graph_merge(self, file_path: str) -> bool:
